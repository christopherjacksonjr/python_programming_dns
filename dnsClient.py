...
def dns_query(type, name, server):
    ...
    server_address = (server, 13331) # Enter Port Number
    ...
    header = struct.pack('!HHHHHH', ID, QR << 15 | OPCODE << 12 | AA << 15 | TC << 15 | RD << 15| RA << 15 | Z << 13 | RCODE, QDCOUNT, ANCOUNT, NSCOUNT, ARCOUNT)
    ...
    qname_encoded = b''.join(qname_encoded_parts) + b'\x80' #enter the closing byte value to signify the end of the domain string (two digits)
    ...
    qclass = QDCOUNT[qtype]     # Lookup the Resource Record class being requested
    ...
    response_question = data[1:1+len(QDCOUNT)] # The data variable starts immediately after the header section, so what is it's index? Note the two '??' '??' will be the same value as we start at a specific index and then go for the entire length of the binary data received.
    assert response_question == question

    # Parse the response answer section
    response_answer = data[1+len(question):] # We would be looking at the same index position as before (after the header)
    offset = 0
